...
TEXTURE_DIR = os.path.join(ROOT_DIR, "textures/flicker")
...
class GenerateSyntheticDataset():
    def __init__(self, config):
        ...
        self.target_object = "buttercookies"
    ...
    def add_object(self, augment_param = None):
        for obj in self.obj_list[:]:
            ...
            if obj.name == "plane":
                ...
                target_file = texture_files[random.randrange(len(texture_files))]
                texture = cv2.cvtColor(cv2.imread(TEXTURE_DIR + "/" + target_file), cv2.COLOR_BGR2RGB)
                # aug = iaa.GaussianBlur(sigma=10)
                # texture = aug.augment_image(texture)
